...
from mvncs_detector import MvNCSObjectDetector
...
try:
    ...
    def play_sound(flag):
        if flag is True:
            sound = pyglet.media.load(BEEP_SOUND_FILE)
            sound.play()
...
class TWD_Demo(object):
    ...
    def run(self, detector, model, rule='bidirectional'):
        """ """
        ...
        vid_out_file  = 'demo.avi'
        self.writer   = cv2.VideoWriter(vid_out_file, fourcc, 10, (640, 480))
        self.cam.setup_scene()
        self.tw_start, self.tw_end = draw_trip_wire(self.cam, self.tw_color)
        self.tw_slope = self._slope(self.tw_end)
        self.detector = MvNCSObjectDetector(model)
        self.detector.set_cam(self.cam)
        self.detector.start(detection_cb=twd_cb)
        self.detector.stop()
        self.writer.release()
...
def main():
    ...
    parser.add_argument('-r', '--res', type=str, default='640x480')
    args = parser.parse_args()
    res_str = args.res
    r1,r2 = res_str.split('x')
    resolution = (int(r1), int(r2))
    print ('Camera Resolution: ', resolution)
    cam_id = args.video if args.video else 0
    demo = TWD_Demo(cam_id, cam_res=resolution)
    demo.run(args.detector, args.model)
